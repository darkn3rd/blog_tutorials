# AWS LBC Demos with the Kubernetes Python client (embedded f-string YAML)

These demos deploy four scenarios using the `kubernetes` client library. Every manifest
here - including the plain Namespace/Deployment/Service ones - is written as a multiline
f-string of YAML, parsed to a dict, and passed as the `body=` argument to the matching typed
client method (`create_namespaced_deployment`, `create_namespaced_service`, ...). Custom
resources (`Gateway`, `TCPRoute`, `LoadBalancerConfiguration`, ...) have no typed
`create_namespaced_<kind>()` method to call, so those fall back to the `DynamicClient`'s
generic apply.
